...
\textbf{The simulator is available under an open source license and can be downloaded from
Github. We added a reference in the outlook section.}
% RÔMULO COMMENT: In this revised paper, we must check if the inclusion of our
% github URL keeps the blind-review process.
...
%\todo[inline, color=green!40]{Jan's answer}
\textbf{We redrew Fig. 3 to make clarify this.}
...
We rewrite the lines 258--264 to make it clearer.}
...
% \todo[inline, color=red!40]{Rômulo's answer}
\textbf{This limitation is explained in lines 376--382. As we described in
...
% \todo[inline, color=red!40]{Rômulo's answer}
\textbf{Thank you for catching this point. We have rewritten the subsection
1.1 to avoid such confusion.}
...
\todo[inline, color=red!40]{Rômulo's answer}
% RÔMULO COMMENT: I added the use of GPU in the abstract, but I'm not sure if we need
% to include limitations and difficulties here.
...
% \todo[inline, color=green!40]{Jan's answer}
\textbf{Thank you. We addressed your consideration in the text and rewrite the
...
% \todo[inline, color=blue!40]{Undefined responsible's answer}
\textbf{The subsection 1.1 was rewritten in the revised manuscript.XXXXXXXX}
...
pipeline on GPU to compute the acoustic frame instead of simulating ray propagation and
interation with the virtual objects. Computing the multipath reflection in sonar simulation
is computationally costly, thus we need to consider the computation time again by modeling
