...
% \todo[inline, color=red!40]{Rômulo's answer}
\textbf{To find the final pixel intensity, we multiply the surface normal angle by the material reflectance $\rho$. This operation takes place in fragment shader. We rewrote the sentences on subsection 3.2 to make the idea clearer. Also, the Fig. 6 was modified to take this consideration.}
...
%%%Rômulo 03/06/2017: Refere-se à última seção do paper "Conclusion and future work". Mudei isso aqui.
...
into account and addressed it as future work in the Section 5.}
...
%%%Rômulo 03/06/2017: A última sentença do texto diz que "Future works will focus on qualitative and time consuming comparison with other sonar simulators and with real acoustic images.". Não sei se é adequado incluir isto no texto desta forma, ou apenas dizendo que os trabalhos futuros incluem comparação e análise detalhadas com imagens reais (como está agora).
\textbf{We just now had the opportunity to record data with an imaging sonar of an object which we can fully simulate (see Figs. 7(c) and 7(f) in the revised paper and Fig. \ref{fig:real_sonar} of this document). Therefore we plan to use this data next months to analyze the quality of the simulation and publish this later when the results are satisfied.}
\newpage
...
% \todo[inline, color=red!40]{Rômulo's answer}
%%% Rômulo 03/06/2017: Reescrevi este parágrafo sobre o problema de limitação.
\textbf{The 8-bit channel representation only causes precision problems with the depth parameter. In this case, we replaced the original depth information by the native GLSL depth buffer, which has 32-bit floating-point values. In fact, this information was not addressed in the first version of the paper. We clarified this point in the revised version.}

% The 8-bit channel representation only causes problem with the 
% The only problem caused by the limitation of the 8-bit channel
...
%%%Rômulo 03/06/2017: eu retirei esse termo do texto para evitar esse tipo de confusão. Onde estava escrito "energy accumulated", eu reescrevi o texto.
...
%%%Rômulo 03/06/2017: acredito que a resposta ainda condiz com o questionamento do revisor.
...
%%%Rômulo 03/06/2017: Eu inclui uma referência no texto sobre isso. Está descrito no final da subseção dos resultados qualitativos.
