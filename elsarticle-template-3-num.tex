...
%%%LUCIANO 24/03/2017: Não botaria isso nessa versão ainda: In addition to our previous work \cite{cerqueira2016}, the normal data can also be defined by a bump mapping technique and material's reflectivity.
%%%ROMULO 24/03/2017: Ok. Faz sentido.
...
\begin{figure*}[t]
    ...
    \captionsetup{justification=centering}
    \caption{Different underwater sonar readings: \subref{fig:swarths:msis} From a mechanical scanning imaging sonar and \subref{fig:swarths:fls} from a forward-looking sonar.}
...
\begin{figure*}[t]
    \centering
    \subfigure[][]{
        \includegraphics[width=0.25\paperwidth]{figs/bump_0}
        \label{fig:bump_0}
    }
    \subfigure[][]{
        \includegraphics[width=0.25\paperwidth]{figs/bump_1}
        \label{fig:bump_1}
    }
    \subfigure[][]{
        \includegraphics[width=0.25\paperwidth]{figs/bump_2}
        \label{fig:bump_2}
    }
    \subfigure[][]{
        \includegraphics[width=0.25\paperwidth]{figs/bump_3}
        \label{fig:bump_3}
    }
    \subfigure[][]{
        \includegraphics[width=0.25\paperwidth]{figs/bump_4}
        \label{fig:bump_4}
    }
    \subfigure[][]{
        \includegraphics[width=0.25\paperwidth]{figs/bump_5}
        \label{fig:bump_5}
    }
    \captionsetup{justification=centering}
    \caption{Example of shader rendering with bump mapping processing: A sphere without texture \subref{fig:bump_0} and with texture \subref{fig:bump_3}; their respective shader image representation in \subref{fig:bump_1} and \subref{fig:bump_4}, where blue is the normal channel and green is the depth one; and the final acoustic image in \subref{fig:bump_2} and \subref{fig:bump_5}. By using bump mapping technique, the textures changes the normal directions, and the sonar image details the appearance of object surface, like in real-world sensing.}
...
\begin{figure}[h]
    ...
    \captionsetup{justification=centering}
    \caption{Examples of different reflectance values, $\rho$, applied in shader image representation, where blue is the normal channel and green is the depth channel: \subref{fig:reflectance:0} raw image; \subref{fig:reflectance:0.35} $\rho = 0.35$; \subref{fig:reflectance:1.40} $\rho = 1.40$; and \subref{fig:reflectance:2.12} $\rho = 2.12$.}
...
%%%Luciano 24/03: Frase esquisita: "the energy is lost two-way in the environment"
%%%Rômulo 24/03: A intensidade em uma imagem do sonar é formada pela porção de energia refletida e capturada pelo dispositivo. Entretanto, a onda acústica perde energia para o meio duas vezes, já que ela é enviada até atingir um objeto e refletida para o sonar novamente (percorre o mesmo caminho duas vezes).
...
%%%Luciano 24/03: o \times é um produto vetoria? x e n não deveriam ser derivadas?
%%%Rômulo 24/03: esta equação é uma element-wise multiplication.
\begin{equation}
\label{eq:2}
y(t) = x(t) \times n(t) \, ,
\end{equation}
...
\begin{figure*}[!ht]
    ...
    \captionsetup{justification=centering}
    \caption{Experiments using mechanical scanning imaging sonar in three different scenarios \subref{fig:msis_scene1}, \subref{fig:msis_scene2} and \subref{fig:msis_scene3}, and the respective processed simulated frames in horizontal axis in \subref{fig:msis_sim1} and \subref{fig:msis_sim2}, and vertical axis in \subref{fig:msis_sim3}.}
    \label{fig:msis}
\end{figure*}
