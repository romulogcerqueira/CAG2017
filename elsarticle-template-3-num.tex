...
\subsection{Evaluation of sonar image quality}

Numerically assessing the performance of a sonar simulator is a non-trivial task. As sonar simulators must work as trustworthy environment to avoid in-field experiments, the goal of the quantitative evaluation should be to demonstrating that the real-world sonar image can be aligned with the synthetic one. This image alignment must carry out by considering a real-world and a virtual scene, both insonified by real and virtual sonar devices, respectively, in the exact same conditions. Since our simulator consider a sonar equipping an AUV, the pose of the vehicle
Among other metrics, image alignment can be inferred by
