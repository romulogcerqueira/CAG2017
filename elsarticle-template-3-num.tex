...
acoustic physics related to the underwater environment. This fact turns the
...
mechanical scanning imaging sonars (MSIS) and foward-looking sonars (FLS). The
...
description files (SDF), which use the SDFormat \cite{sdformat2017}, an XML
...
\begin{figure*}[t]
    ...
    A sphere without texture \subref{fig:bump_0} and with texture
    \subref{fig:bump_3}; their respective shader image representation
    in \subref{fig:bump_1} and \subref{fig:bump_4}, where blue is the
    normal channel and green is the depth one; and the final acoustic
    image in \subref{fig:bump_2} and \subref{fig:bump_5}. By using
...
the reflectivity and texture were set to the target. The rotation of the the
...
contains a grid around the AUV (see Fig. \ref{fig:msis_scene2}), an MSIS in
...
($2.5 min$). For MSIS type, to the best of our knowledge, there is any
...
the qualitative results, the sonar simulator can be by feature detection
...
focus on qualitative and computation\hyp{}efficiency evaluations with
other imaging sonar simulators.
