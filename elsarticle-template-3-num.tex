...
%%%Luciano: mudei de "three 2D parameters" -> "three parameters"
%%% to simulate the sonar device => "render sonar device"
...
\begin{itemize}[]
    \item \textbf{Distance / Depth} is the camera focal length, calculated by the Euclidean distance to the object surface point; 
...
%%%Lucian: Essa frase "At the end of sonar rendering step, the shader provides a 3-channel matrix composed by intensity, depth and angular distortion parameters." parece confundir tudo. Esses parâmetros não eram usados para renderizar o sonar a partir da câmera virtual? Para que dizer que "the shader provides a 3-channel matrix..."????? Precisa dizer alguma coisa sobre o quanto esses parâmetros afetam a renderização, etc. Pense nisso!!!! 
...
% ----------------------------------------------------------------------------------
%%%Mudei ligeiramente este título! "operation of the sonar device"
...
% ----------------------------------------------------------------------------------
%%%coloquei essa subseção como subseção da 3.3, pois ainda se refere a sonar operation
